...
pub const RSC_KEY_HEADERS: &[&str] = &["rsc", "next-router-prefetch", "next-router-state-tree", "next-url"];
...
mod tests {
    ...
    fn get<'a>(path: &'a str, h: &'a HeaderMap) -> RequestMetadata<'a> {
        RequestMetadata { method: &Method::GET, host: "shop.example.com", path, query: None, headers: h }
    }
    ...
    fn prefetch_rsc_is_coalesced_but_never_stored() {
        ...
        let hints = NextJs.classify_request(&get("/products/iphone", &h));
        assert!(hints.coalesce_only, "state-tree cardinality makes storing these pointless");
    }
    ...
    fn draft_mode_bypasses_even_on_a_public_looking_path() {
        let mut h = HeaderMap::new();
        h.insert(header::COOKIE, HeaderValue::from_static("foo=1; __prerender_bypass=xyz"));
    ...
    fn draft_cookie_match_is_exact_not_substring() {
        ...
        let mut h = HeaderMap::new();
        h.insert(header::COOKIE, HeaderValue::from_static("not__prerender_bypass_x=1"));
